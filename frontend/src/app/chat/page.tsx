                    ...
                    {msg.sources.map((src, idx) => (
                      <div key={idx} className="group relative">
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 text-xs font-medium cursor-help border border-indigo-100 dark:border-indigo-800/50">
                          ...
                          {src.filename} (Chương {src.chapter})
                        </span>
                        {/* Tooltip hiển thị trích đoạn */}
                        <div className="absolute bottom-full left-0 mb-2 w-64 p-3 bg-slate-800 text-white text-xs rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10 overflow-hidden line-clamp-5">
                          {src.content}
                        </div>
                      </div>
                    ))}
