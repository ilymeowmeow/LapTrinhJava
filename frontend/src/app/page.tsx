...
export default function Home() {
  ...
  const fetchChatSessions = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Lọc bỏ các cuộc trò chuyện rỗng (chưa có tin nhắn nào)
        const nonEmptySessions = data.filter((s: any) => s.messages && s.messages.length > 0);
        setChatSessionsList(nonEmptySessions);
      }
    } catch (e) {
      console.error("Lỗi lấy danh sách lịch sử chat:", e);
    }
  };

  useEffect(() => {
    // Khởi tạo session: lấy từ localStorage hoặc tạo mới
    const initSession = async () => {
      try {
        const storedId = localStorage.getItem("chatSessionId");
        if (storedId) {
          setCurrentSessionId(parseInt(storedId));
          // Lấy lại lịch sử chat từ Backend
          const res = await fetch(`http://localhost:8080/api/chat/session/${storedId}/messages`);
          if (res.ok) {
            ...
            if (msgs && msgs.length > 0) {
              ...
              setMessages(mapped);
              return; // Thoát nếu khôi phục thành công
            }
          }
        }
        
        // Nếu không có storedId hoặc không lấy được lịch sử, tạo session mới
        ...
        if (data.id) {
          ...
          localStorage.setItem("chatSessionId", data.id.toString());
          fetchChatSessions();
        }
      } catch (e) {
  ...
  const handleGenerateScript = async () => {
    try {
      const res = await fetch("http://localhost:8080/api/finetuning/generate-script", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      fetchChatSessions(); // Cập nhật lại sidebar sau khi có tin nhắn mới
    } catch (error) {
      setMessages(prev => [...prev, {
        role: "bot",
    ...
    <div className="flex h-screen w-full font-sans bg-[#F9FAFB] text-slate-800">
      ...
      <aside className={`${isSidebarOpen ? 'w-[280px]' : 'w-0'} flex flex-col bg-[#FAFAFA] border-r border-slate-200/60 transition-all duration-300 overflow-hidden shrink-0 z-20`}>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-2 space-y-2">
          <button
            ...
            className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl text-[15px] font-medium transition-all ${activeView === 'documents' ? 'bg-white shadow-sm border border-slate-200/60 text-slate-800' : 'text-slate-600 hover:bg-slate-100'
              }`}
          >
          ...
          <button
            ...
            className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl text-[15px] font-medium transition-all ${activeView === 'chat' ? 'bg-white shadow-sm border border-slate-200/60 text-slate-800' : 'text-slate-600 hover:bg-slate-100'
              }`}
          >
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className="opacity-80">
          ...
          <button
            ...
            className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl text-[15px] font-medium transition-all ${activeView === 'finetuning' ? 'bg-white shadow-sm border border-slate-200/60 text-slate-800' : 'text-slate-600 hover:bg-slate-100'
              }`}
          >
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className="opacity-80">
          ...
          <button
            ...
            className="w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl text-[15px] font-medium transition-all text-slate-600 hover:bg-slate-100"
          >
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className="opacity-80">
          ...
          {activeView === "chat" && chatSessionsList.length > 0 && (
            <div className="mt-6 pt-4 border-t border-slate-200/60">
              <div className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-3 px-4">Lịch sử trò chuyện</div>
              <div className="space-y-1">
                {chatSessionsList.map((s: any) => (
                  <div key={s.id} className={`group flex items-center justify-between px-4 py-2.5 rounded-xl cursor-pointer transition-colors ${currentSessionId === s.id ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-slate-100'}`}
                    onClick={async () => {
                      setCurrentSessionId(s.id);
                      localStorage.setItem("chatSessionId", s.id.toString());
                      try {
                        const res = await fetch(`http://localhost:8080/api/chat/session/${s.id}/messages`);
                        if (res.ok) {
                          const msgs = await res.json();
                          if (msgs && msgs.length > 0) {
                  ...
                  >
                    <div className="flex items-center gap-3 overflow-hidden">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0 opacity-70"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path></svg>
                      <div className="truncate text-[13px] font-medium">{s.title || `Trò chuyện #${s.id}`}</div>
                    </div>
                    <button 
                      onClick={async (e) => {
                        e.stopPropagation();
                        if(confirm("Bạn có chắc chắn muốn xóa cuộc trò chuyện này khỏi Database?")) {
                          try {
                            await fetch(`http://localhost:8080/api/chat/session/${s.id}`, { method: "DELETE" });
      ...
      <main className="flex-1 flex flex-col relative h-full bg-white">
        ...
        {activeView === "finetuning" && (
          <div className="flex-1 overflow-y-auto pt-20 p-8 max-w-5xl mx-auto w-full">
            <h2 className="text-2xl font-bold text-slate-800 mb-8">Module Fine-Tuning (PEFT/LoRA)</h2>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
              {/* Configuration Form */}
              <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
                <h3 className="text-lg font-bold text-slate-800 mb-4">Cấu hình Hyperparameters</h3>

                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1">Base Model (HuggingFace ID)</label>
                    <input type="text" value={ftModelName} onChange={e => setFtModelName(e.target.value)} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-[#7C3AED]" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1">Tên file Dataset (JSONL)</label>
                    <input type="text" value={ftDatasetName} onChange={e => setFtDatasetName(e.target.value)} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-[#7C3AED]" />
                  </div>
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      <label className="block text-sm font-medium text-slate-600 mb-1">LoRA R</label>
                      <input type="number" value={ftLoraR} onChange={e => setFtLoraR(Number(e.target.value))} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-[#7C3AED]" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-600 mb-1">LoRA Alpha</label>
                      <input type="number" value={ftLoraAlpha} onChange={e => setFtLoraAlpha(Number(e.target.value))} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-[#7C3AED]" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-600 mb-1">Epochs</label>
                      <input type="number" value={ftEpochs} onChange={e => setFtEpochs(Number(e.target.value))} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-[#7C3AED]" />
                    </div>
                  </div>
                  <div className="pt-2">
            ...
            {generatedScript && (
              <div className="bg-slate-900 rounded-3xl overflow-hidden shadow-xl">
                <div className="bg-slate-800 px-6 py-3 flex items-center justify-between border-b border-slate-700">
                  <span className="text-sm font-medium text-slate-300">train_peft.py</span>
                  <button onClick={() => { navigator.clipboard.writeText(generatedScript); alert("Đã copy script!") }} className="text-xs px-3 py-1.5 bg-slate-700 hover:bg-slate-600 text-white rounded-lg transition-colors">
                    Copy Code
                  </button>
                </div>
                <div className="p-6 overflow-x-auto">
                  <pre className="text-sm text-green-400 font-mono"><code>{generatedScript}</code></pre>
                </div>
              </div>
            )}
          </div>
